...
import antlr4.ILPMLgrammar2tme4Parser.LoopContext;
...
import antlr4.ILPMLgrammar2tme4Parser.VariableAssignContext;
...
protected IASTfactory factory;
	...
	protected IASTexpression[] toExpressions(
			List<antlr4.ILPMLgrammar2tme4Parser.ExprContext> args) {
		if (args == null) return new IASTexpression[0];
		IASTexpression[] r = new IASTexpression[args.size()];
		int pos = 0;
		for (antlr4.ILPMLgrammar2tme4Parser.ExprContext e : args) {
			r[pos++] = e.node;
		}
		return r;
	}
	...
	protected IASTblock.IASTbinding[] toBindings(
			List<Token> vars, 
			List<antlr4.ILPMLgrammar2tme4Parser.ExprContext> vals) {
	...
	// TME4 BY AMINE BENSLIMANE
	
	@Override
	public void enterUnless(UnlessContext ctx) {
		System.out.println("Unless détecté ! -- Traitement dans ILPMLListener");
	}

	@Override
	public void exitUnless(UnlessContext ctx) {

		// On considère la syntaxe "body unless condition" comme "if !condition then body"
		
		ctx.node = factory.newAlternative(
				factory.newUnaryOperation(factory.newOperator("!"), ctx.condition.node), 
				ctx.body.node,
				factory.newStringConstant("false"));
	}
